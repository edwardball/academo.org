var demo = new Demo({
	...
	$(window, ".js-planet").mouseup(function(){
	...
	$(".js-orbit-demo").mousedown(function(e){
		...
		if (!_this.isDragging){
			...
			angle = Math.atan2(_this.vy,_this.vx);
			modulus = Math.sqrt(Math.pow(_this.vy,2) + Math.pow(_this.vx,2))
	  ...
	  $("#demo, .js-planet, .js-drag-line").on("mousemove touchmove", function(e){
	  	if (_this.isDragging){
		  	...
		  	$(".js-drag-line").attr("d", "M0,0l"+modulus+",0l0,-5l10,5l-10,5,l0,-5").attr("transform", "translate("+_this.x +" "+_this.y+") rotate("+angle*180/Math.PI+")");
		  	// _this.vx = (eoffsetX-width/2) - _this.x;
		  	// _this.vy = (eoffsetY-height/2 - _this.y);
		  	_this.vx = modulus*Math.cos(angle);
	...
	force: function(distance){
		...
		return {x:-20000 * this.ui.mass.value * 1.85 * distance.x / Math.pow(distanceAbs,3), y:-20000* this.ui.mass.value*1.85 * distance.y / Math.pow(distanceAbs,3)}
	},
	...
	update: function(e){
		...
		if (e == "zoom"){
			...
			angle = Math.atan2(this.vy,this.vx);
			modulus = Math.sqrt(Math.pow(this.vy,2) + Math.pow(this.vx,2))
			$(".js-drag-line").attr("d", "M0,0l"+modulus+",0l0,-5l10,5l-10,5,l0,-5").attr("transform", "translate("+_this.x +" "+_this.y+") rotate("+angle*180/Math.PI+")");
		...
		var trailString = "M";
		 ...
		 var step = this.ui.mass.value > 200 ? 1 : 3;

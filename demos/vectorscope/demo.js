var ui = {
    inputType: {
        title: "Input",
        value: "lissajous",
        values: [["Lissajous","lissajous"], ["Hypotrochoid","hypotrochoid"], ["Rhodonea","rhodonea"]]
    },
    freeze: {
    ...
    b: {
        title: "b parameter (for hypotrochoid)",
        value: 2.5,
        range:[0,10],
        resolution:0.1,
    },
...
var osc1,osc2,gain,delay, delay2;
...
for (var i = 0 ; i < 3.14159 * 2 ; i+=0.001){
  ...
  bufferDataL[index] = x * -0.1 * scaleFactor + 10;
  bufferDataR[index] = y * -0.1 * scaleFactor;
...
function startRoseCurveOscs(){
  ...
  roseGain1.connect(audioContext.destination);
  roseGain2.connect(audioContext.destination);
...
function updateAudioSource(type){
  if (type == "Lissajous"){
...
function drawData(){
    ...
    for (var i = -analyser.frequencyBinCount/2; i <= analyser.frequencyBinCount/2; i++) {
        ...
        // if (streaming == true){
            ...
            var yc = ui.gain.value * ((timeDomainR[index]))*200/(ui.volts.value);
            // yc += c.height/2;
            var xc = ui.gain.value * ((timeDomainL[index]))*200/(ui.volts.value);
